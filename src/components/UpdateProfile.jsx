...
s_a[35] = " Adra | Alipurduar | Amlagora | Arambagh | Asansol | Balurghat | Bankura | Bardhaman | Basirhat | Berhampur | Bethuadahari | Birbhum | Birpara | Bishanpur | Bolpur | Bongoan | Bulbulchandi | Burdwan | Calcutta | Canning | Champadanga | Contai | Cooch Behar | Daimond Harbour | Dalkhola | Dantan | Darjeeling | Dhaniakhali | Dhuliyan | Dinajpur | Dinhata | Durgapur | Gangajalghati | Gangarampur | Ghatal | Guskara | Habra | Haldia | Harirampur | Harishchandrapur | Hooghly | Howrah | Islampur | Jagatballavpur | Jalpaiguri | Jhalda | Jhargram | Kakdwip | Kalchini | Kalimpong | Kalna | Kandi | Karimpur | Katwa | Kharagpur | Khatra | Krishnanagar | Mal Bazar | Malda | Manbazar | Mathabhanga | Medinipur | Mekhliganj | Mirzapur | Murshidabad | Nadia | Nagarakata | Nalhati | Nayagarh | Parganas | Purulia | Raiganj | Rampur Hat | Ranaghat | Seharabazar | Siliguri | Suri | Takipur | Tamluk";
    ...
    const [profileInfoObj,setProfileInfoObj]=useState({
        username:"",
        mobile:"",
        address:"",
        city:"",
     ...
     useEffect(() => {
        ...
        if (!selectedFile) {
            setPreview(`../uploads/${profileInfoObj.ProfilePic}`);
        }
        else{
        const objectUrl = URL.createObjectURL(selectedFile);
        setPreview(objectUrl);
        }
        if (!selectedFileAadhar) {
            setPreviewAadhar(`../uploads/${profileInfoObj.AadharPic}`);
        }
        else{
        const objectUrl = URL.createObjectURL(selectedFileAadhar);
        setPreviewAadhar(objectUrl);
        }
        document.getElementById('username').value=profileInfoObj.username;
        document.getElementById('mobile').value=profileInfoObj.mobile;
        document.getElementById('gender').value=profileInfoObj.gender;
        document.getElementById('city').defaultValue=profileInfoObj.city;
        document.getElementById('homestate').value=profileInfoObj.homestate;
        document.getElementById('bloodGrp').value=profileInfoObj.bloodGrp;
        document.getElementById('address').value=profileInfoObj.address;       
    }, [selectedFile,selectedFileAadhar,profileInfoObj.ProfilePic,profileInfoObj.AadharPic]);
    const [errMob,setMobErr]=useState("");
    const [errCity,setCityErr]=useState("PLEASE SELECT A STATE FIRST");
    ...
    function UpdateProfileObj(event){
        ...
        if(name==='homestate'&&value!=='undefined'){
            // alert(state_arr.indexOf(value));
            setStateErr('');
            document.getElementById('city').disabled=false;
            document.getElementById('cityDef').hidden=true;
            document.getElementById('city').style.color='white';
            setCityErr('');
            let option_str=document.getElementById('city');
            city_arr = s_a[state_arr.indexOf(value)+1].split(" | ");
            for (var i = 0; i < city_arr.length; i++) {
                option_str.options[option_str.length] = new Option(city_arr[i], city_arr[i]);
            }
    }
        else{
            setStateErr('SELECT A VALID STATE');
        }

    }
    async function submitRec(){
        // alert(JSON.stringify(profileObj))
        if(profileObj.homestate==='undefined'){
        ...
        const MobChk=/^[7-9][0-9]{9}$/;
        if(MobChk.test(profileObj.mobile)){
        // alert(profileObj.ProfilePic);
        // alert(profileObj.AadharPic);
        setMobErr("");
        ...
        .catch((error)=>{
            ...
            setMobErr("ENTER CORRECT FORMAT OF MOBILE NUMBER");
        }
    }
    function UpdateProfilePic(e){
        // alert(e.target.name);
        // alert(JSON.stringify(e.target.files[0].name));
        setProfileObj({...profileObj,[e.target.name]:e.target.files[0]});
    ...
    function UpdateAadharPic(e){
        setProfileObj({...profileObj,[e.target.name]:e.target.files[0]});
        if (!e.target.files || e.target.files.length === 0) {
            setSelectedFileAadhar(undefined)
            return
        }
    ...
    <motion.div exit={{opacity:0}} animate={{opacity:1}} initial={{opacity:0}}>
        ...
        <div className="container">
            <center><h1><u>UPDATE PROFILE</u></h1></center>
            <Row>
                <Col md={7}>
                <Row>
                    <Col md={12}>
                    ...
                    <Form.Control type="text" name="username" id="username"  className='formInput' onChange={UpdateProfileObj}/>
                    </Form.Group>
                    </Col>
                </Row>
                <Row>
                    <Col md={4}>
                    <Form.Group>
                    <Form.Label className="formTitle">MOBILE NUMBER</Form.Label>
                    <Form.Control type="text" name="mobile" id='mobile'  onChange={UpdateProfileObj} className='formInput'/>
                    ...
                    <Form.Control type="text" name="gender" id="gender" onChange={UpdateProfileObj} className='formInput'/>
                    <Form.Text className="text-muted">
                    </Form.Text>
                    </Form.Group> 
                    </Col>
                    <Col md={4}>
                    <Form.Group>
                    <Form.Label className="formTitle">BLOOD GROUP</Form.Label>
                    <Form.Control type="text" name="bloodGrp" id="bloodGrp" onChange={UpdateProfileObj} className='formInput'/>
                    <Form.Text className="text-muted">
                    </Form.Text>
                    </Form.Group> 
                    </Col>
                ...
                <Row>
                    ...
                    <Form.Control type="text" name="address" id="address" onChange={UpdateProfileObj} className='formInput'/>
                    </Form.Group> 
                    </Col>
                </Row>
                <Row>
                <Col md={6}>
                    ...
                    {/* <Form.Control type="text" id="homestate" name="homestate"onChange={UpdateProfileObj} className='formInput'/> */}
                    <Form.Control as="select"className='selectdiv' name='homestate' id='homestate' custom onChange={UpdateProfileObj}>
                    ...
                    <Col md={6}>
                    <Form.Group>
                    <Form.Label className="formTitle">CITY</Form.Label>
                    <Form.Control as="select"className='selectdiv' name='city' disabled id='city' custom onChange={UpdateProfileObj}>
                        <option value='undefined' id='cityDef' style={{color:'black,',fontWeight:'bolder'}}>{profileObj.city}</option>
                    </Form.Control>
                    {/* <Form.Control style={{color:'black'}} type="text" name="city" id="city" disabled onChange={UpdateProfileObj} className='formInput'/> */}
                    <Form.Text style={{color:'red'}}>{errCity}</Form.Text>
                    </Form.Group>
                    </Col>
                </Row>
                <Row>
                    <Col md={6}>
                    <Form.Group>
                    <Form.Label className="formTitle">CHOOSE PROFILE PIC</Form.Label>
                    <Form.Control type="file" name="ProfilePic" onChange={UpdateProfilePic} className='formInput2'/>
                    </Form.Group> 
                    </Col>
                    <Col md={6}>
                    <Form.Group>
                    <Form.Label className="formTitle">CHOOSE AADHAR CARD</Form.Label>
                    <Form.Control type="file" name="AadharPic" onChange={UpdateAadharPic}  className='formInput2'/>
                    </Form.Group> 
                    </Col>
                </Row>
                ...
                <Col md={5}>
                   <center> <Row>
                        <Col md={12}>
                        <center><span style={{color:"green"}}>PROFILE PIC</span></center>
                            <div className="profilePic">
                                <img src={preview} alt="PROFILE PIC" width="100%" height="100%"></img>
                            </div>
                        </Col>
                        </Row>
                        <Row>
                            <Col md={12}>
                            <center><span style={{color:"green"}}>AADHAR PIC</span></center>
                            <div className="aadharPic">
                            <img src={previewAadhar} alt="AADHAR PIC" width="100%" height="100%"></img>
                            </div>
                        </Col>
                        </Row></center>
                </Col>
